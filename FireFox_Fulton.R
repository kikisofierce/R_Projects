...
#Project:Automate Apartment Add Posting to Facebook (Phase1)
#scrap fulton grace website for pcitures and text

# find PATH of Python Installer 
#import sys
...
description_text <- paste(description_text, url, sep="\n")
...
write_csv(fulton_data , file = "C:/Users/Open/Documents/FultonGrace/FultonGrace/Phase1//fulton_data.csv")
  ...
  # extract virtual tour link fields
  #link_vt <- read_html(html) %>% # parse HTML
   # html_nodes(".s-listing_content-interior-subitem:nth-child(1) .s-listing_content-interior-subitem-link") %>%
    #html_text %>%
    #print()
  
  
  # extract youtube tour link fields
  #link_yt <- read_html(html) %>% # parse HTML
   # html_nodes(".s-listing_content-interior-subitem+ .s-listing_content-interior-subitem .s-listing_content-interior-subitem-link") %>%
    #html_text %>%
    #print()
